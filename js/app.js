...
// slider_about было вместо pageSlider
...
let pageSlider = new Swiper('.page__slider', {
	...
	*/
	...
	// События
	on: {
		// События автоинициализации
		init: function () {
			menuSlider();
			setScrollType();
			wrapper.classList.add('_loaded');
		},
		// События смены слайдера
...
function setScrollType() {
	...
	for (let index = 0; index < pageSlider.slides.length; index++) {
		...
		const pageSlideContent = pageSlide.querySelector('.slide__content');
		if (pageSlideContent) {
...
let mapFooterCurrent = document.querySelector('.map-footer__current');
if (mapFooterCurrent) {
	let go = true;
	mapFooterCurrent.addEventListener("click", function (e) {
		if (go) {
			go = false;
			langHeaderCurrent.classList.toggle('_active');
			_slideToggle(mapFooterCurrent.nextElementSibling);
	...
	document.addEventListener("click", function (e) {
		if (!e.target.classList.contains('map-footer__current') && !e.target.classList.contains('map-footer__items')) {
			mapFooterCurrent.classList.remove('_active');
			_slideUp(mapFooterCurrent.nextElementSibling);
		}
